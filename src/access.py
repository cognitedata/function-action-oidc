...
def missing_basic_capabilities(client: CogniteClient, project: str, cred_name: str) -> List[str]:
    missing = []
    logger.error(f"Running fn 'missing_basic_capabilities': {project=}, {cred_name=}")
    logger.error(f"- can we fool github redact? {' _ '.join(project)=}")
    try:
        token_inspect = inspect_token(client)
        logger.error(f"{token_inspect.projects=}")
        # logger.error(f"{token_inspect=}")
        # inspect/token endpoint will not fail if credentials also have access to another CDF project:
        logger.error("Can we fool github redact again?")
        for p in token_inspect.projects:
            logger.error(f"{p.url_name}, {' _ '.join(p.url_name)}")
